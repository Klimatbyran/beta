...
import rawPoliticalRule from './politicalRule.json'

const RawPoliticalRule = rawPoliticalRule as {
  kommun: string
  styre: string
  other: string
}[]
...
export class PoliticalRuleService {
  public getPoliticalRule(municipalityName: string): Array<string> {
    const rawMunicipality = RawPoliticalRule.find(
      (rawPM: any) =>
        rawPM.kommun.toLowerCase() ===
          `${municipalityName.toLowerCase()} kommun` ||
        rawPM.kommun.toLowerCase() ===
          `${municipalityName.toLowerCase()}s kommun` ||
        rawPM.kommun.toLowerCase() ===
          `${municipalityName.toLowerCase()} stad` ||
        rawPM.kommun.toLowerCase() ===
          `${municipalityName.toLowerCase()}s stad` ||
        rawPM.kommun.toLowerCase() ===
          `region ${municipalityName.toLowerCase()}` ||
