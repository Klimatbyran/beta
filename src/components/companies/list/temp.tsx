...
import { X, BarChart3, PieChart as PieChartIcon, Filter, ArrowLeft, BarChart2 } from 'lucide-react';
import { Company, SECTOR_NAMES } from '../types';
          ...
          <div className="space-y-3">
            <div className="space-y-1">
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm text-white">{scope1.toLocaleString()} tonnes CO₂e</span>
              </div>
            ...
            <div className="space-y-1">
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm text-white">{scope2.toLocaleString()} tonnes CO₂e</span>
              </div>
              <div className="h-2 bg-black-1 rounded-full overflow-hidden">
                <div
            ...
            <div className="space-y-1">
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm text-white">{scope3.toLocaleString()} tonnes CO₂e</span>
              </div>
              <div className="h-2 bg-black-1 rounded-full overflow-hidden">
                <div
...
const CustomTooltip = ({
  ...
  const scope1 = payload.find(p => p.dataKey === `${sector}_scope1`)?.value || 0;
  const scope2 = payload.find(p => p.dataKey === `${sector}_scope2`)?.value || 0;
  const scope3 = payload.find(p => p.dataKey === `${sector}_scope3`)?.value || 0;
  const total = scope1 + scope2 + scope3;
...
const PieChartTooltip: React.FC<TooltipProps<number, string>> = ({ active, payload }) => {
  if (!active || !payload || !payload.length) return null;
  ...
  return (
    <div className="bg-black-2 border border-black-1 rounded-lg shadow-xl p-4 text-white">
      ...
      <div className="text-sm text-grey">
        <div>{Number(value).toLocaleString()} tonnes CO₂e</div>
...
const sectorColors = {
  "10": { base: "#F48F2A", scope1: "#FDE7CE", scope2: "#FDB768", scope3: "#F48F2A" },
  "15": { base: "#59A0E1", scope1: "#D4E7F7", scope2: "#99CFFF", scope3: "#59A0E1" },
  "20": { base: "#AAE506", scope1: "#F1FFCC", scope2: "#D5FD63", scope3: "#AAE506" },
  "25": { base: "#F0759A", scope1: "#FAE1E9", scope2: "#EEA0B7", scope3: "#F0759A" },
  "30": { base: "#F48F2A", scope1: "#FDE7CE", scope2: "#FDB768", scope3: "#F48F2A" },
  "35": { base: "#59A0E1", scope1: "#D4E7F7", scope2: "#99CFFF", scope3: "#59A0E1" },
  "40": { base: "#AAE506", scope1: "#F1FFCC", scope2: "#D5FD63", scope3: "#AAE506" },
  "45": { base: "#F0759A", scope1: "#FAE1E9", scope2: "#EEA0B7", scope3: "#F0759A" },
  "50": { base: "#F48F2A", scope1: "#FDE7CE", scope2: "#FDB768", scope3: "#F48F2A" },
  "55": { base: "#59A0E1", scope1: "#D4E7F7", scope2: "#99CFFF", scope3: "#59A0E1" },
  "60": { base: "#AAE506", scope1: "#F1FFCC", scope2: "#D5FD63", scope3: "#AAE506" },
};
...
const EmissionsChart: React.FC<EmissionsChartProps> = ({ companies, selectedSectors }) => {
  ...
  const chartData = useMemo(() => {
    if (selectedSector) {
      ...
      return Array.from(years).sort().map(year => {
        ...
        sectorCompanies.forEach(company => {
          const periodForYear = company.reportingPeriods.find(
            period => period.startDate.startsWith(year)
          );

          if (periodForYear?.emissions) {
            const scope1 = periodForYear.emissions.scope1?.total || 0;
            const scope2 = periodForYear.emissions.scope2?.calculatedTotalEmissions || 0;
            const scope3 = periodForYear.emissions.scope3?.calculatedTotalEmissions || 0;
            const total = scope1 + scope2 + scope3;
    ...
    return Array.from(years).sort().map(year => {
      ...
      if (chartType === 'stacked-total') {
        selectedSectors.forEach(sectorCode => {
          const sectorName = SECTOR_NAMES[sectorCode as keyof typeof SECTOR_NAMES];
  ...
  const pieChartData = useMemo(() => {
    if (selectedSector) {
      ...
      return sectorCompanies.map(company => {
        const periodForYear = company.reportingPeriods.find(
          period => period.startDate.startsWith(selectedYear)
        );
    ...
    const totalEmissions = sectorTotals.reduce((sum, sector) => sum + sector.value, 0);
  ...
  const handleBarClick = (data: any) => {
    ...
    if (selectedSector) {
      ...
      const year = data.activeLabel;
      const sectorCode = Object.entries(SECTOR_NAMES).find(([_, name]) => name === sector)?.[0];
      if (sectorCode) {
  ...
  const years = useMemo(() => Array.from(new Set(chartData.map(d => d.year))).sort(), [chartData]);
    ...
    <div className="w-full space-y-6">
      <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg focus:outline-none transition-colors ${
                chartType === 'bar' ? 'bg-black-1 text-white' : 'text-grey hover:text-white'
              }`}
            >
            ...
            <button
              onClick={() => setChartType('stacked-total')}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg focus:outline-none transition-colors ${
                chartType === 'stacked-total' ? 'bg-black-1 text-white' : 'text-grey hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg focus:outline-none transition-colors ${
                chartType === 'pie' ? 'bg-black-1 text-white' : 'text-grey hover:text-white'
              }`}
            >
        ...
        <div className="flex items-center gap-2 text-sm text-grey">
          ...
          {selectedSector ? (
            <span>
              Showing companies in {SECTOR_NAMES[selectedSector as keyof typeof SECTOR_NAMES]}
            </span>
          ) : (
            <span>
              Showing {selectedSectors.length} of {Object.keys(SECTOR_NAMES).length} sectors
            </span>
          )}
        </div>
      </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          {chartType === 'pie' ? (
            ...
            >
              ...
              />
              <YAxis
                label={{ value: 'CO₂e (tonnes)', angle: -90, position: 'insideLeft', fill: '#888888' }}
                tick={{ fill: '#888888' }}
              />
              <Tooltip content={selectedSector ? <CompanyTooltip /> : <CustomTooltip />} />
              {chartType === 'stacked-total' ? (
                selectedSectors.map(sectorCode => {
                  const sectorName = SECTOR_NAMES[sectorCode as keyof typeof SECTOR_NAMES];
                  const colors = sectorColors[sectorCode as keyof typeof sectorColors];
                  return (
                    <Bar
                      key={sectorCode}
                      dataKey={sectorName}
                ...
                companies
                  .filter(company => company.industry?.industryGics.sectorCode === selectedSector)
                  .map((company, index) => {
