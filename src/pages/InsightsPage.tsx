...
function BlogMeta({ category, date, readTime }: { category: string; date: string; readTime: string }) {
  return (
    <div className="flex items-center gap-4">
      <span className="px-3 py-1 bg-blue-5/50 rounded-full text-blue-2 text-sm">{category}</span>
  ...
  return (
    ...
    >
      ...
      <div className="p-8 space-y-4">
        <BlogMeta category={post.category} date={post.date} readTime={post.readTime} />
        <Text variant="h3" className="group-hover:text-blue-2 transition-colors">{post.title}</Text>
...
export function InsightsPage() {
  const featuredPost = blogMetadata[0];
  const otherPosts = blogMetadata.slice(1);
  ...
  return (
    <div className="max-w-[1200px] mx-auto space-y-16">
      ...
      <div className="relative rounded-level-1">
        <Link to={`/insights/${featuredPost.id}`} className="group block transition-all duration-300">
          ...
          <div className="relative h-[500px] overflow-hidden rounded-level-1">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-16 space-y-4">
              <BlogMeta category={featuredPost.category} date={featuredPost.date} readTime={featuredPost.readTime} />
