...
export function SectorComparison({ 
  ...
  const sortedCompanies = [...sectorCompanies].sort((a, b) => 
    (b.metrics?.emissionsReduction || 0) - (a.metrics?.emissionsReduction || 0)
  );
  
  // Find the current company's rank
  const currentCompanyRank = sortedCompanies.findIndex(c => 
    c.wikidataId === currentCompany.wikidataId
  ) + 1;

  // Get reporting years for the current company
  const reportingYears = currentCompany.reportingPeriods
  ...
  const companiesWithScopes = sortedCompanies.map((company, index) => {
    ...
    const reportedCategories = latestPeriod?.emissions?.scope3?.categories?.map(cat => cat.category) || [];
  ...
  const sectorName = currentCompany.industry?.industryGics?.sv?.sectorName || 
                    currentCompany.industry?.industryGics?.sectorName;
  ...
  return (
    <div className={cn(
      "bg-black-2 rounded-level-1 p-16",
      className
    )}>
      <div className="flex items-center justify-between mb-12">
        <div className="space-y-2">
          ...
          <Text variant="muted">
            Plats {currentCompanyRank} av {sectorCompanies.length} företag i sektorn
          </Text>
        </div>
        <div className="flex items-center gap-4">
          <Select value={view} onValueChange={(value) => setView(value as ComparisonView)}>
