import { Building2, Package, Factory, Truck, Trash2, Bus, Car, Box, TrendingDown, Wrench, Recycle, Home, Search } from 'lucide-react';
import type { LucideIcon } from 'lucide-react';

// Category metadata combining icons and colors
export const categoryMetadata: Record<number, {
  icon: LucideIcon;
  color: string;
  name: string;
  description: string;
...
export const getCategoryFilterColors = (category: number) => {
  const color = getCategoryColor(category).replace('var(--', '').replace(')', '');
