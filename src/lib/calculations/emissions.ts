...
export function analyzeTrend(
  ...
  const firstNonZeroPeriod = features.guessBaseYear
    ? sortedPeriods.find(
        (period) => period.emissions?.calculatedTotalEmissions > 0,
      )
    : sortedPeriods[0]
...
export function projectEmissions(
  ...
  const historicalPoints = analysis.trend.dataPoints.filter(
    (point) => point.value > 0,
  )
  ...
  return Array.from({ length: years }, (_, i) => {
    ...
    const trend = Math.max(0, firstNonZeroValue * Math.pow(1 + dampedChange, i))
