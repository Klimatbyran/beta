 ...
 */
export function interpolateScope3Categories(periods: EmissionPeriod[]): EmissionPeriod[] {
  ...
  return sortedPeriods.map((period, index) => {
    if (!period.emissions?.scope3?.categories || index === 0 || index === sortedPeriods.length - 1) {
      return period;
    }

    const prevPeriod = sortedPeriods[index - 1];
    const nextPeriod = sortedPeriods[index + 1];
    const currentCategories = new Set(period.emissions.scope3.categories.map(c => c.category));
    ...
    allCategories.forEach(category => {
      ...
      const prevValue = prevPeriod.emissions?.scope3?.categories?.find(c => c.category === category)?.total;
      const nextValue = nextPeriod.emissions?.scope3?.categories?.find(c => c.category === category)?.total;
...
export function analyzeTrend(
  ...
  const firstNonZeroPeriod = features.guessBaseYear
    ? sortedPeriods.find(period => period.emissions?.calculatedTotalEmissions > 0)
    : sortedPeriods[0];
  ...
  if (!firstNonZeroPeriod) {
    ...
    const endYear = new Date(sortedPeriods[sortedPeriods.length - 1].endDate).getFullYear();
  ...
  const totalEmissions = relevantPeriods
    .filter(period => period.emissions?.calculatedTotalEmissions > 0)
    .map(period => ({
      year: new Date(period.startDate).getFullYear(),
      value: period.emissions!.calculatedTotalEmissions
    }));

  const scope12Emissions = relevantPeriods
    .filter(period => 
      (period.emissions?.scope1?.total || 0) + 
      (period.emissions?.scope2?.calculatedTotalEmissions || 0) > 0
    )
    .map(period => ({
      year: new Date(period.startDate).getFullYear(),
  ...
  const hasScope3 = relevantPeriods.some(p => p.emissions?.scope3?.calculatedTotalEmissions);
  const scope3Count = relevantPeriods.filter(p => p.emissions?.scope3?.calculatedTotalEmissions).length;
  const scope3Coverage = scope3Count / relevantPeriods.length;
  const consistentScope3 = hasScope3 && scope3Coverage > 0.8;

  // Calculate standard deviation
  const totalStdDev = calculateStandardDeviation(totalEmissions.map(e => e.value));
  const scope12StdDev = calculateStandardDeviation(scope12Emissions.map(e => e.value));
  ...
  if (consistentScope3 && scope3Coverage > 0.8) {
    ...
    trendType = 'total';
    dataQuality = 'high';
    message = 'Trendberäkningen baseras på totala utsläpp (scope 1-3) med hög tillförlitlighet.';
    ...
    trendType = 'scope12';
    dataQuality = 'medium';
    message = 'Trendberäkningen baseras endast på scope 1-2 utsläpp då scope 3 data är inkonsekvent.';
  } else {
    bestTrend = {
      slope,
      r2: 0.5,
    ...
    dataQuality = 'low';
    message = 'Trendberäkningen har låg tillförlitlighet på grund av stora variationer eller bristfällig data.';
  }
  ...
  return {
    trend: {
      ...
      endYear: totalEmissions[totalEmissions.length - 1]?.year || firstNonZeroYear,
    },
...
export function projectEmissions(
  ...
  // Find first non-zero value to use as reference
  const firstNonZeroValue = analysis.trend.dataPoints.find(point => point.value > 0)?.value || startValue;
  
  // Calculate initial annual percentage change from historical data
  const historicalPoints = analysis.trend.dataPoints.filter(point => point.value > 0);
  ...
  if (historicalPoints.length >= 2) {
    ...
    if (yearDiff > 0) {
      ...
      initialAnnualChange = Math.pow(lastPoint.value / firstPoint.value, 1/yearDiff) - 1;
      
      // Limit initial change to reasonable bounds (-15% to +15% per year)
      initialAnnualChange = Math.max(Math.min(initialAnnualChange, 0.15), -0.15);
    }
  }
...
export function getDataQualityColor(quality: 'high' | 'medium' | 'low'): string {
  switch (quality) {
    case 'high': return 'var(--green-3)';
    case 'medium': return 'var(--orange-3)';
    case 'low': return 'var(--pink-3)';
  }
}